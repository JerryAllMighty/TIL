...
- 7 ~ 17까지다.
[전체 개요]
 자바 8에서 17까지의 주요 기능들에 대해 설명해 주세요.
 자바 버전을 선택할 때 고려해야 할 요소는 무엇입니까?
 자바 17버전에서 추가된 주요 기능은 무엇인가요?
 자바 8과 자바 11의 주요 차이점과 개선된 기능에 대해 설명해주세요.
 자바 8 이상에서 제네릭과 관련된 새로운 기능이나 개선 사항에는 어떤 것들이 있나요?
 자바 버전을 업그레이드할 때 고려해야 할 주요 사항은 무엇인가요?
 자바 8 이상의 버전에서 추가된 기능 중 코딩 테스트에 유용했던 기능은 무엇이었나요?
 자바 17의 새로운 기능과 그 장점은 무엇인가요?
 자바 8과 자바 9의 기본 GC에 대해 설명해주세요.
 자바 8의 람다 함수와 스트리밍 기능을 설명해보세요.
[본론]
# Java 7
## switch 에 문자열 사용가능
enum / int만 쓰던 제약 해소
...
발행-구독(Publisher-Subscriber) 모델을 표준화한 API입니다. 
RxJava나 Project Reactor 같은 리액티브 라이브러리들이 상호 호환될 수 있는 규격(Flow.Publisher, Flow.Subscriber 등)을 제공합니다.
...
## 내부 API 접근 제한:
그동안 "어차피 되니까" 관례처럼 쓰던 sun.misc.Unsafe 같은 내부 API에 대한 접근을 차단하여, 자바 플랫폼의 안정성을 높였습니다.
JDK 내부 API 접근 제한
→ “어차피 되니까 쓰자” 문화에 제동
...
## 컬렉션 팩토리 메서드 List.of(), Set.of(), Map.of()
...
## 컴파일 없이 실행
javac 없이 java HelloWorld.java 명령만으로 소스 파일을 즉시 실행할 수 있게 되어, 가벼운 스크립트 작성용으로 자바를 쓰기 좋아졌습니다.
...
Java 14 이후 (Helpful NPE):
```
Cannot invoke "Address.getCity()" because the return value of "Details.getAddress()" is null
```
해석: "Details.getAddress()의 반환값이 null이라서 getCity()를 호출할 수 없어요!"라고 정확히 집어줍니다.

# 자바 15
## Text Blocks (Standard): """
멀티라인 문자열이 정식으로 도입되었습니다.
