...
<div markdown="1">
 mutable이라고 문제에서 가정했으면 final로 선언이 안되었다고 가정하겠습니다. 불변 상태를 유지하려면 생성 이후에 수정이 되지 않게 관리하는게 중요하다고 생각합니다. 수정자 메서드가 있다면 사용하지 못하게 접근제어자를 바꾸는 등의 조치를 취할 수 있을 것 같고
필드 변수도 외부에서 접근하지 못하게 private 접근 제어할 것 같습니다.
</div>
</details>