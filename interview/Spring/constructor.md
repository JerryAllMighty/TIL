...
2. DI(의존성 주입)의 3가지 방법과 차이점은?
   text
   생성자 주입 / 필드 주입 / Setter 주입
2. Spring에서 생성자 주입을 권장하는 이유 3가지?
   text
1. 순환 참조 방지 (BeanCurrentlyInCreationException 발생)
2. 불변성 보장 (final 필드 가능)
3. 테스트 용이 (new로 직접 주입 가능)
3. 필드 주입(@Autowired 필드)의 단점 3가지?
   text
1. 순환 참조 시 애플리케이션 시작됨 (런타임 오류)
2. 불변성 없음 (Setter처럼 변경 가능)
3. 프레임워크 의존성 (테스트 어려움)
   꼬리 질문 (딥 다이브)
...
10. @ComponentScan과 생성자 주입 순서?
    text
1. 컴포넌트 스캔 → 2. 빈 등록 → 3. 생성자 주입 → 4. @PostConstruct
   실무 꼬리 질문
